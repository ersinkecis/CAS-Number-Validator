/****** Object:  StoredProcedure [dbo].[f_is_valid_cas_number]    Script Date: 15.10.2019 01:21:38 ******/
CREATE FUNCTION [dbo].[f_is_valid_cas_number](@p_cas_number AS VARCHAR(50)) --'1234-56-7'
RETURNS INT
AS
...
BEGIN
  ...
  BEGIN
    ...
    BEGIN
      ...
      BEGIN
          SET @i = @i + 1
          SET @onechar = SUBSTRING(@checksum_source, @i, 1);
          IF (CHARINDEX(@onechar, '1234567890') = 0)
          BEGIN
            SET @is_format_valid = 0;
          END
          SET @temp = CONVERT(NUMERIC, @onechar)  * @i;
